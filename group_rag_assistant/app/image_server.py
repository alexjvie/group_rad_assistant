...
app = FastAPI(title="SEM Image Analysis", version="0.5")
...
def _detect_bottom_legend_strip_top(gray_u8: np.ndarray) -> Optional[int]:
    ...
    Robust for typical SEM UI overlays: wide dark region spanning most of the width.

    Returns y_top (int) in image coordinates, or None if not detected.
    """
    h, w = gray_u8.shape[:2]
    ...
    # A legend strip tends to have low median AND lots of dark pixels across width
    ok = (med < 80) & (dark_frac > 0.40)

    # Scan from bottom upward for a sustained run
    min_run = max(18, int(0.015 * h))  # at least ~1.5% of height
    run = 0
    for i in range(ok.size - 1, -1, -1):
        if ok[i]:
            ...
            if run >= min_run:
                # top of the run is i+1 within region
                return start + i + 1
            run = 0
...
def _find_scale_bar_bbox(gray_u8: np.ndarray) -> Optional[Dict[str, Any]]:
    ...
    Returns bbox in FULL image coords:
      {x,y,w,h,score,polarity}
    """
    gray = gray_u8
    h, w = gray.shape[:2]

    # Prefer bottom area; if a legend strip exists, search inside it.
...
def detect_scale_bar_nm_per_px(file_bytes: bytes) -> Dict[str, Any]:
    """
    - finds scale bar bbox
    - OCRs ROIs optimized for SEM legend: around bar + right box + full legend strip chunk
    - returns nm/px
    """
    img = _decode_image(file_bytes)
    gray = _to_gray(img)

    # detect on resized for speed
    ...
    # legend strip (original) to create a "legend ROI"
    strip_top = _detect_bottom_legend_strip_top(gray)
    if strip_top is None:
    ...
    # 1) tight around bar (includes label above)
    rois.append((
        "around_bar",
        (
    ...
    # 2) right-of-bar box area (very common)
    rois.append((
        "right_box",
        (
    ...
    # 3) chunk of legend strip around bar (captures "10 µm" even if layout is weird)
    rois.append((
        "legend_chunk",
        (
...
def _exclude_legend_from_mask(mask: np.ndarray, gray_an: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    # (B) scale bar + label box (in case strip not detected or partial overlays)
...
def analyze_sem(
    ...
    *,
    ...
    min_area_px: int,
    hole_area_px: int,
    mode: str,  # "both" | "area" | "length"
) -> Dict[str, Any]:
    mode = (mode or "both").strip().lower()
    if mode not in {"both", "area", "length"}:
        mode = "both"
    do_area = mode in {"both", "area"}
    do_len = mode in {"both", "length"}
    ...
    raw_mask = blur < thr  # dark foreground by default
    if invert:
        raw_mask = ~raw_mask
    ...
    # draw excluded rects in blue for debugging
    ...
    out: Dict[str, Any] = {
        ...
        # previews for UI (will be removed from JSON panel on frontend)
    ...
    if do_area:
        ...
        out["area_fraction_foreground"] = float(fg_frac)
        out["area_fraction_background"] = float(1.0 - fg_frac)
    else:
        out["area_fraction_foreground"] = None
        out["area_fraction_background"] = None
    ...
    if do_len:
        ...
        lengths_px.sort()
        out["stats_px"] = _stats(lengths_px)
        out["length_hist_px"] = _hist(lengths_px, bins=18)

        if nm_per_px_eff and nm_per_px_eff > 0:
            um_per_px = nm_per_px_eff / 1000.0
            lengths_um = [x * um_per_px for x in lengths_px]
            out["stats_um"] = _stats(lengths_um)
            out["length_hist_um"] = _hist(lengths_um, bins=18)
        else:
            out["stats_um"] = None
            out["length_hist_um"] = None
    else:
        out["stats_px"] = None
        out["length_hist_px"] = None
        out["stats_um"] = None
        out["length_hist_um"] = None

    # Summary (kurz)
    summary: Dict[str, Any] = {}
    if out["area_fraction_foreground"] is not None:
        summary["foreground_area_fraction"] = out["area_fraction_foreground"]
    if out["stats_px"] is not None:
        summary["n_objects"] = out["stats_px"]["n"]
        summary["mean_length_px"] = out["stats_px"]["mean"]
        summary["median_length_px"] = out["stats_px"]["median"]
        if out["stats_um"] is not None:
            summary["mean_length_um"] = out["stats_um"]["mean"]
            summary["median_length_um"] = out["stats_um"]["median"]
    out["summary"] = summary
...
<head>
  ...
  <style>
    :root{
      ...
      --pillBg:#f3f4f6; --pillText:#111111;
      --pillActiveBg:#111111; --pillActiveText:#ffffff;
      --inputBg:#ffffff;
      --contentMax: 980px;
    }
    [data-theme="dark"]{
      ...
      --pillBg:rgba(255,255,255,0.08); --pillText:#e5e7eb;
      --pillActiveBg:#e5e7eb; --pillActiveText:#0b0f19;
    ...
    *{ box-sizing:border-box; }
    ...
    .pill{ display:flex; gap:6px; padding:6px; border-radius:999px; background:var(--pillBg); border:1px solid var(--border); }
    .pill button{ border:0; background:transparent; color:var(--pillText); padding:8px 12px; border-radius:999px; cursor:pointer; font-size:13px; font-weight:650; user-select:none; }
    .pill button.active{ background:var(--pillActiveBg); color:var(--pillActiveText); }
  ...
  <div class="shell">
    ...
    <div class="topbar">
      <div class="brand">
        ...
        <div>
          ...
          <div class="subtitle">Enter startet Analyze · Legend/Scale wird automatisch ausgeschlossen</div>
        </div>
      </div>
    ...
    <div class="panel">
      <div class="centerHero">
        ...
        <p class="heroP">
          <b>Both</b> = Maske + Porosity + Length. Die gesamte SEM-Legende unten (schwarzer Balken + Text + Scale-Bar-Box)
          wird aus der Analyse entfernt, damit nichts verfälscht wird.
        </p>

        <div class="metaRow">
          <div class="pill" role="tablist" aria-label="mode selector">
            <button id="btn-both" class="active" onclick="setMode('both')">Both</button>
            <button id="btn-area" onclick="setMode('area')">Porosity</button>
      ...
      <div class="grid">
        ...
        <div class="preview">
          <div class="card" id="fileCard" style="display:none;">
            ...
            <div class="kv"><span>Mode</span><span id="modeLabel"></span></div>
          ...
          <div class="cards" id="metricsCards" style="display:none;">
            ...
            <div class="card" id="cardLen" style="display:none;">
...
function setMode(m){
  ...
  document.getElementById("modeLabel").textContent = (m==="both") ? "Both" : (m==="area" ? "Porosity" : "Length");
}
...
fileInput.addEventListener("change", (e) => {
  ...
  document.getElementById("modeLabel").textContent = (mode==="both") ? "Both" : (mode==="area" ? "Porosity" : "Length");
...
async function autoScale(){
  ...
  try{
    ...
    // show compact JSON (remove preview b64)
    const compact = {...j};
...
async function analyze(){
  ...
  try{
    ...
    const showArea = (j.area_fraction_foreground !== null);
    const showLen  = (j.stats_px !== null);

    document.getElementById("cardArea").style.display = showArea ? "block" : "none";
    document.getElementById("cardLen").style.display  = showLen ? "block" : "none";
    ...
    // IMPORTANT: JSON panel should show data only (remove base64 blobs)
    const compact = {...j};
